...
module.exports = {
  ...
  squawk: function(text, bot_id) {
    ...
    return constructResponseObject(mentionedMembers.user_ids, lociArray, finalString, bot_id);
  }
};
  ...
  for (let id in ids) {
    for (let member in members) {
      if (ids[id] == members[member].user_id) {
        mentionedMembers.nicknames.push(members[member].nickname);
        mentionedMembers.user_ids.push(members[member].user_id);
      }
    }
  }
...
function constructResponseString(mentionedMembers) {
  ...
  for (let member in mentionedMembers.nicknames) {
    finalString += `@${mentionedMembers.nicknames[member]} `;
  }
  return finalString;
}
...
function getMentionLoci (finalString, mentionedMembers) {
  ...
  for (let letter in finalString) {
    if (finalString[letter] == '@') {
      ...
      let memberLength = mentionedMembers.nicknames[x].length + 1; // + 1 to account for @ symbol
